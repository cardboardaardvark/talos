...
static void count_physical_pages(const multiboot::v1_memory_map_t *map, void *arg)
{
    ...
    if (map->type != MULTIBOOT1_MMAP_TYPE_AVAILABLE) return;
    ...
    assert(map->address % PAGE_SIZE == 0);

    std::uintptr_t region_start = map->address;
    std::uintptr_t region_end = region_start + map->length;

    libk::writef_log("\tMemory region: 0x%p - 0x%p\n", region_start, region_end);

    for(size_t offset = 0; region_start + offset < region_end; offset += PAGE_SIZE) {
...
static void * init_physical_pages(uint64_t length, uint64_t map_address)
{
    ...
    auto kernel_end = reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(&_link_end) + PAGE_SIZE);

    physical_page_visitor_args_t visitor_args = {
        .kernel_end = kernel_end,
    };

    libk::writef_log("kernel_end=0x%p map_address=0x%p length=0x%p\n", kernel_end, map_address, length);
    ...
    libk::writef_log("Found %u physical pages; new kernel end: 0x%p\n", visitor_args.num_pages, kernel_end);

    libk::writef_log("Available physical memory: %u KiB\n", hal::available_physical_pages() * PAGE_SIZE / 1024);
...
static hal::page_directory_t identity_map_kernel_memory(hal::page_directory_t page_directory, void *kernel_end)
{
    ...
    assert(x86::read_cr0() & CR0_ENABLE_PROTECTED_FLAG);
    assert(kernel_end != nullptr);
    assert(libk::is_page_aligned(kernel_end));

    const auto map_start = PAGE_SIZE;

    libk::writef_log("Identity mapping from 0x%p to 0x%p\n", map_start, kernel_end);

    // Identity map everything from the 2nd page to the end of the kernel
    // in memory.
    for (uintptr_t address = map_start; address <= reinterpret_cast<uintptr_t>(kernel_end); address += PAGE_SIZE) {
        libk::map_identity_page(page_directory, address, PAGE_FLAG_PRESENT | PAGE_FLAG_RW);
    }
...
static void map_shared_virtual(hal::page_directory_t page_directory)
{
    ...
    libk::writef_log("Mapping shared area 0x%p:0x%p -> 0x%p:0x%p\n", shared_start_physical, shared_end_physical - 1, shared_start_virtual, shared_end_virtual - 1);

    for (auto physical_page = shared_start_physical; physical_page < shared_end_physical; physical_page += PAGE_SIZE) {
        auto virtual_page = physical_page + shared_offset;
        libk::map_virtual_page(page_directory, virtual_page, physical_page, PAGE_FLAG_PRESENT | PAGE_FLAG_RW);
    }
}
...
{
    auto kernel_guard_page = reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(kernel_end) + PAGE_SIZE);
    auto kernel_heap = reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(kernel_guard_page) + PAGE_SIZE);
...
// Invoked by crt0.asm
extern "C" void init(uint32_t multiboot_magic, const multiboot::v1_info_t *multiboot_info)
{
    x86::init_gdt();

    init_console();

    libk::write_log("IBM PC platform initializing\n");

    if (multiboot_magic != MULTIBOOT1_MAGIC) libk::panic("Invalid multiboot 1 magic number: 0x%x\n", multiboot_magic);
    ...
    libk::write_log("Platform initialization complete\n");
}
